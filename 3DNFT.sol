...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    function addAssetType(uint64 _assetType, uint64 _maxAmount, uint64 _coinIndex, string memory _copyright, bytes memory _sig) public isManager {
        require(_maxAmount > 0);
        uint8 mId = 3;
        bytes32 taskHash = keccak256(abi.encode(_assetType, _maxAmount, _coinIndex, _copyright, taskIndex, mId));
        verifyApproval(taskHash, _sig);
        initAssets( _assetType, _maxAmount, _coinIndex, _copyright);
    }
    
    function modifyAssetType(uint64 _typeId, uint64 _maxAmount, uint64 _coinIndex, string memory _copyright, bytes memory _sig) public isManager {
        uint8 mId = 4;
        bytes32 taskHash = keccak256(abi.encode(_typeId, _maxAmount, _coinIndex, _copyright, taskIndex, mId));
        verifyApproval(taskHash, _sig);
    ...
    function addCoin(uint256 _coinIndex, address _tokenAddress, string memory _tokenSymbol, string memory _tokenName, bytes memory _sig) public isManager {
        uint8 mId = 5;
        bytes32 taskHash = keccak256(abi.encode(_coinIndex, _tokenAddress, _tokenSymbol, _tokenName, taskIndex, mId));
        verifyApproval(taskHash, _sig);
